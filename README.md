...
## get the pre-built KMER file, Genomes file or KMER collection file
...
* Sample HTML report (Illumina): http://opengene.org/fastv/fastv.html
* Sample JSON report: http://opengene.org/fastv/fastv.json
* If the `KMER` file is specified, there will be a `POSITIVE` or `NEGATIVE` result, which is determined by comparing the mean depth of the KMER keys to the threshold (`--positive_threshold`).
...
```
  ...
  -c, --kmer_collection               the unique KMER collection file in fasta format, see an example: http://opengene.org/kmer_collection.fasta (string [=])
  -k, --kmer                          the unique KMER file of the detection target in fasta format. data/SARS-CoV-2.kmer.fa will be used if none of KMER/Genomes/KMER_Collection file is specified (string [=])
  -g, --genomes                       the Genomes file of the detection target in fasta format. data/SARS-CoV-2.genomes.fa will be used if none of KMER/Genomes/KMER_Collection file is specified (string [=])
